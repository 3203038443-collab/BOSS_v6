(function() {
  ...
  function scanAll() {
    ...
    function isStatusLine(text) {
      text = (text || "").trim();
      return text.indexOf("[已读]") === 0 || text.indexOf("[送达]") === 0;
    }
    ...
    function isBadName(text) {
      ...
      if (text.indexOf("已读") >= 0 || text.indexOf("送达") >= 0) return true;
      if (text.length > 12) return true;
       if (/^[\u4e00-\u9fa5]{5,}$/.test(text)) return true;
      if (/[，。！？,.!?:：]/.test(text)) return true;
      if (/(最近|工作|简历|老板|回复|方便|您好|你好)/.test(text)) return true;
      if (/^(校园合伙人|牛人|Boss|BOSS|招聘者|候选人|沟通|搜索)$/.test(text)) return true;
      return false;
    }
    ...
    function getPreviewLine(lines) {
      for (var i = 0; i < lines.length; i++) {
        var line = (lines[i] || "").trim();
        if (isStatusLine(line)) return line;
      }
      for (var j = lines.length - 1; j >= 1; j--) {
        var tail = (lines[j] || "").trim();
        if (tail && !isTimeLike(tail)) return tail;
      }
      return "";
    }
    ...
    function getHeaderLine(lines) {
      for (var i = 0; i < lines.length; i++) {
        ...
        if (!line || isTimeLike(line) || isStatusLine(line)) continue;
        return line;
      }
      return "";
    }
    ...
    function extractName(item) {
      try {
        ...
        for (var li = 0; li < lines.length; li++) {
          if (isTimeLike(lines[li]) || isStatusLine(lines[li])) { hasContext = true; break; }
        }
        if (!hasContext && lines.length < 2) return null;
        var headerLine = getHeaderLine(lines);
        var name = cleanCandidateName(headerLine);
        if (!name || name.length < 1 || name.length > 20 || /^\d+$/.test(name) || isTimeLike(name) || isBadName(name)) return null;
        var lastMsg = getPreviewLine(lines).slice(0, 80);
        var unreadNum = 0;
    ...
    if (result.candidates.length === 0) {
      ...
      for (var i = 0; i < allEls.length; i++) {
        ...
        if (!hasContext && lines.length < 2) continue;
        var name = cleanCandidateName(getHeaderLine(lines));
        if (!name || name.length > 20 || name.length < 1 || /^\d+$/.test(name) || isTimeLike(name) || isBadName(name) || seen[name]) continue;
        seen[name] = true;
        result.candidates.push({name: name.slice(0,20), last_msg: getPreviewLine(lines).slice(0,80), has_unread: false, unread_count: 0, has_read: getPreviewStatus(lines) === true, x: Math.round(r.left + r.width/2), y: Math.round(r.top + r.height/2)});
      }
    }
  ...
  async function scanRecommendTalentsDeep() {
    ...
    for (var round = 0; round < 5; round++) {
      ...
      if (current.candidates.length > 0) {
        best.debug = current.debug + "|round:" + round;
        return best;
      }
      try {
        scrollOnce();
    ...
    if (best) best.debug = (best.debug || "recommend_scan") + "|deep_retry";
  ...
  async function navigatePage(url) {
    try {
      ...
      console.log("[CT] navigatePage error:", e.message || e);
      return false;
    }
  }
